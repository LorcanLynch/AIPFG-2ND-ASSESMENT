...
std::vector<aipfg::scene> makeAllScenes(std::vector<aipfg::Sprite*>);
aipfg::transition makeTransition(aipfg::GameManager&, Vector2, int);
int main(int argc, char* argv[])
{
    ...
    std::vector<scene> scenes = makeAllScenes(all_game_chars);
    gm.SetScenes(scenes);
    gm.ChangeScene(0);
...
aipfg::NPC make_reaper(aipfg::GameManager& gm, raylib::Texture& tex)
{
    ...
    return { gm, s, name, { nature, gambit },1 };
}
...
aipfg::NPC make_green_knight(aipfg::GameManager& gm, raylib::Texture& tex)
{
    ...
    return { gm, s, name, { nature, gambit },0 };
}

aipfg::transition makeTransition(aipfg::GameManager& gm, Vector2 pos, int index, int activeIndex)
{
    return { pos,index,activeIndex,gm };
}

std::vector<aipfg::scene> makeAllScenes(std::vector<aipfg::Sprite*> sprites)
{
    using namespace aipfg;

    scene startScene = { LoadTiled("../resources/ashlands.json"),{sprites[0],sprites[1]} };


    scene scene2 = { LoadTiled("../resources/ashlands.json"),{sprites[0],sprites[2] }
    };
    std::cout << sprites.size();
    return { startScene,scene2 };
